		...
		<div class="row <?=$_prefix;?>row  is--wrap">
			<div class="cols <?=$_prefix;?>cols  <?=$_mod;?>  is--name">
				<div class="<?=$_prefix;?>item">
					<input type="text" class="<?=$_prefix;?>control form-control validate[required, custom[onlyLetterSp]]  <?=$_mod;?>  <?=$_color;?>" id="<?=$_id;?>[name]" name="f[Имя]" placeholder="Ваше имя">
				</div>	
			</div>
			...
			<div class="cols <?=$_prefix;?>cols  <?=$_mod;?>  is--tel">
				<div class="<?=$_prefix;?>item">
					<input type="text" class="<?=$_prefix;?>control form-control validate[required]  <?=$_mod;?>  <?=$_color;?>" id="<?=$_id;?>[products]" name="f[Продукция]" placeholder="Продукция">
				</div>	
			</div>
			<div class="cols <?=$_prefix;?>cols  <?=$_mod;?>  is--agreement">
				<div class="rect-switch <?=$_prefix;?>item  <?=$_mod;?>  <?=$_color;?>">
					...
					<label for="<?=$_id;?>[processing]" class="rect-switch__text  <?=$_color;?>"> 
						Я согласен на обработку <a href="<?=l(278);?>">персональных данных</a>
					</label>
				</div>	
			</div>
