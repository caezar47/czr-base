	...
	<div class="<?=$param["block_prefix"];?>dropdown">
		<div class="<?=$param["block_prefix"];?>dropdown-inner">
			<ul class="<?=$param["block_prefix"];?>nav">
				<li class="<?=$param["block_prefix"];?>item <?if($this->post['id'] == 6){echo ' is--active ';}?> ">
				...
				<?
					foreach($cat_list as $cat) {
						...
						$title = $cat->name;
						$item_class = '';
						if($this->entity && $this->entity->term_id && $this->entity->term_id == $cat->term_id) {
							$item_class = $item_class . ' is--active ';
						}
						?>
						<li class="<?=$param["block_prefix"];?>item <?=$item_class;?> ">
